...
for($i=0;$i<count($auditores);$i++){
	$datos1[$i]['NOMBRE'] = $auditores[$i]['NOMBRE'].' '.$auditores[$i]['APELLIDO_PATERNO'].' '.$auditores[$i]['APELLIDO_MATERNO'];
	$datos1[$i]['ID']  = $auditores[$i]['ID'];	
	for($j=0;$j<12;$j++){
		// DIAS TRABAJADOS POR AUDITOR
		$m='';
		$m= substr('0'.($j+1),-2);
		$consulta1 = "SELECT COUNT(DT.ID)  AS COUNT_DT FROM(
		...
		$resp1 = $database->query($consulta1)->fetchAll();
		...
		$datos1[$i]['TASA_OCUPACIONAL'][$j] = number_format(($resp1[0]['COUNT_DT']+$DIAS_VACAC)*100/$dias_habiles_x_mes[$j],2);
