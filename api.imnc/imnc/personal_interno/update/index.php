...
if ($CURP != '') {
    $count_curp = $database->count("PERSONAL_INTERNO", ["AND"=>["CURP" => $CURP,"NO_EMPLEADO[!]"=>$NO]]);
    if ($count_curp > 0) {
...
if ($NO_SEGURIDAD != '') {
    $count_curp = $database->count("PERSONAL_INTERNO", ["AND"=>["NO_SEGURO_SOCIAL" => $NO_SEGURIDAD,"NO_EMPLEADO[!]"=>$NO]]);
    if ($count_curp > 0) {
...
$id = $database->update("PERSONAL_INTERNO", [
	"NOMBRE"=>$NOMBRE,
