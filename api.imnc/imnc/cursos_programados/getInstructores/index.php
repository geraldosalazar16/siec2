...
valida_parametro_and_die($option, "Es necesario uan opcion");
...
{
    $query ="SELECT * FROM
		(SELECT  PT.ID,CONCAT(PT.NOMBRE,' ',PT.APELLIDO_PATERNO,' ',PT.APELLIDO_MATERNO) AS NOMBRE,PT.STATUS,PTR.ID AS ID_ROL, PTR.ROL,1 AS ISROL ,'' AS ID_CURSO, '' AS NOMBRE_CURSO,0 AS ISCURSO
		FROM PERSONAL_TECNICO PT INNER JOIN PERSONAL_TECNICO_CALIFICACIONES PTC ON PT.ID = PTC.ID_PERSONAL_TECNICO 
		LEFT JOIN PERSONAL_TECNICO_ROLES PTR ON  PTC.ID_ROL = PTR.ID 
		WHERE PTR.ID = 7
		UNION ALL
		SELECT  PT.ID,CONCAT(PT.NOMBRE,' ',PT.APELLIDO_PATERNO,' ',PT.APELLIDO_MATERNO) AS NOMBRE,PT.STATUS,'' AS ID_ROL, '' AS ROL, 0 AS ISROL, C.ID_CURSO, C.NOMBRE,1 AS ISCURSO
		FROM PERSONAL_TECNICO PT 
		LEFT JOIN PERSONAL_TECNICO_CALIFICACIONES PTC ON PT.ID = PTC.ID_PERSONAL_TECNICO
		LEFT JOIN PERSONAL_TECNICO_CALIF_CURSOS PTCC ON  PTC.ID = PTCC.ID_PERSONAL_TECNICO_CALIFICACION 
		LEFT JOIN CURSOS C ON PTCC.ID_CURSO = C.ID_CURSO
