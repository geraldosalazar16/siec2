...
function creacion_expediente_registro($id_registro, $entidad, $rutaExpediente, $database){
	...
	$tipo_tabla = $database->query("SELECT EX_TABLA_ENTIDADES.ID as ID, EX_TABLA_ENTIDADES.DESCRIPCION AS Tipo_Entidad, EX_TABLA_ENTIDADES.TABLA AS Tabla_Consultar FROM EX_TABLA_ENTIDADES where EX_TABLA_ENTIDADES.ID=".$database->quote($entidad))->fetchAll(PDO::FETCH_ASSOC);
	
	
	$consulta_tabla=$tipo_tabla[0]["Tabla_Consultar"];
	$consulta_nombre=$tipo_tabla[0]["Tipo_Entidad"];
	
	$tipo_clientes = $database->query("SELECT $consulta_tabla.RFC AS Nombre_rfc, $consulta_tabla.ID from $consulta_tabla WHERE ID = $id_registro ")->fetchAll(PDO::FETCH_ASSOC);
	...
	$id_entidad = $tipo_tabla[0]["ID"];
	if($id_entidad == 3){//es un prospecto
		
		$direc = $rutaExpediente."/".$consulta_nombre."/".$cliente."/Citas/";
		$direc = str_replace(" ","_",$direc);
		if (file_exists($direc)) {
    			//echo "El fichero $direc existe";
		}
...
function crea_instancia_expedientes_registro($id_registro,$entidad,$database){
	...
	for($k = 0 ; $k < count($expedientes); $k++){
		...
		valida_error_medoo_and_die("EX_REGISTRO_EXPEDIENTE","");
	...
	WHERE ID_EXPEDIENTE = ".$database->quote($id_expediente))->fetchAll(PDO::FETCH_ASSOC);
		...
		for($j = 0 ; $j < count($tipo_documentos);$j++){
			...
			valida_error_medoo_and_die("EX_ARCHIVO_EXPEDIENTE",""); 
		}
