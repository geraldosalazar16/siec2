...
$valores = $database->query("SELECT `I_EC_AUDITORIAS`.`ID_SERVICIO_CLIENTE_ETAPA`,`I_EC_AUDITORIAS`.`DURACION_DIAS`, `I_SG_AUDITORIAS_TIPOS`.`TIPO`,`I_SG_AUDITORIA_STATUS`.`STATUS`,`I_EC_AUDITORIAS`.`ID_USUARIO_CREACION`,`I_EC_AUDITORIAS`.`ID_USUARIO_MODIFICACION`,`I_EC_AUDITORIAS`.`TIPO_AUDITORIA`,`I_EC_AUDITORIAS`.`STATUS_AUDITORIA`,`I_EC_AUDITORIAS`.`CICLO` FROM `I_EC_AUDITORIAS` INNER JOIN `I_SG_AUDITORIAS_TIPOS` ON `I_SG_AUDITORIAS_TIPOS`.`ID` = `I_EC_AUDITORIAS`.`TIPO_AUDITORIA` INNER JOIN `I_SG_AUDITORIA_STATUS` ON `I_SG_AUDITORIA_STATUS`.`ID` = `I_EC_AUDITORIAS`.`STATUS_AUDITORIA` WHERE `I_EC_AUDITORIAS`.`ID_SERVICIO_CLIENTE_ETAPA`= ".$id." AND `I_EC_AUDITORIAS`.`CICLO`= ".$ciclo)->fetchAll(PDO::FETCH_ASSOC);
...
FROM 
    ...
    INNER JOIN `I_EC_AUDITORIAS` ON `I_EC_AUDITORIAS`.`ID_SERVICIO_CLIENTE_ETAPA`=`I_SG_AUDITORIA_SITIOS`.`ID_SERVICIO_CLIENTE_ETAPA` AND `I_EC_AUDITORIAS`.`TIPO_AUDITORIA` = `I_SG_AUDITORIA_SITIOS`.`TIPO_AUDITORIA` 
...
FROM 
    ...
    INNER JOIN `I_EC_AUDITORIAS` ON `I_EC_AUDITORIAS`.`ID_SERVICIO_CLIENTE_ETAPA`=`I_SG_AUDITORIA_GRUPOS`.`ID_SERVICIO_CLIENTE_ETAPA` AND `I_EC_AUDITORIAS`.`TIPO_AUDITORIA` = `I_SG_AUDITORIA_GRUPOS`.`TIPO_AUDITORIA` 
