...
$i_ec_auditorias = $database->get("I_EC_AUDITORIAS", "*", ["AND"=>["ID_SERVICIO_CLIENTE_ETAPA"=>$id_sce,"TIPO_AUDITORIA"=>$id_tipo_auditoria,"CICLO"=>$ciclo]]);
valida_error_medoo_and_die();
// SG_TIPOS_SERVICIO
//$sg_tipo_servicio = $database->get("SG_TIPOS_SERVICIO", "*", ["ID"=>$sg_auditorias["ID_SG_TIPO_SERVICIO"]]);
$ec_tipo_servicio = $database->select("I_TIPOS_SERVICIOS",
...
if($etapa_proceso["ETAPA"] == 'Inicial'){
	$i_ec_auditorias["INICIAL"] = 1;
}
else{
	$i_ec_auditorias["INICIAL"] = 0;
}
...
if ($completo == "true") { // Realiza consultas adicionales para regresar un reporte completo
		...
		// EC_TIPOS_SERVICIO => TIPOS_SERVICIO
		...
		$servicio_cliente_etapa["TIPO_SERVICIO"] = $tipo_servicio;
			...
			$servicio_cliente_etapa["INFO_SERVICIO"] = $ec_tipo_servicio;
		//$ec_tipo_servicio["SERVICIO_CLIENTE_ETAPA"] = $servicio_cliente_etapa;

	$i_ec_auditorias["SERVICIO_CLIENTE_ETAPA"] = $servicio_cliente_etapa;
...
print_r(json_encode($i_ec_auditorias));
?> 
