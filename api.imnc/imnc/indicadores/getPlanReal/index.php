...
$str_fechas1 .= ' (';
$str_fechas2 .= ' ';
$str_fechas_acumulado1 .= ' (';
$str_fechas_acumulado2 .= ' ';
foreach ($columnas as $index=>$s)
{
	if($index==4)
	{
		$str_fechas2 .= "CSF.".$s." BETWEEN '".$fechas_inicio."' AND '".$fechas_fin."' ";
		$str_fechas_acumulado2 .= "CSF.".$s." BETWEEN '".$fechas_inicio_acumulado."' AND '".$fechas_fin_acumulado."' ";
	}

		$str_fechas1 .= "CSF.".$s." BETWEEN '".$fechas_inicio."' AND '".$fechas_fin."' OR ";
	    $str_fechas_acumulado1 .= "CSF.".$s." BETWEEN '".$fechas_inicio_acumulado."' AND '".$fechas_fin_acumulado."' OR ";


}
$str_fechas1 = substr($str_fechas1, 0, -4);
$str_fechas1 .= ') ';
$str_fechas_acumulado1 = substr($str_fechas_acumulado1, 0, -4);
$str_fechas_acumulado1 .= ') ';
//$str_fechas1 .= ') AND ISNULL(CSF.FECHA_FIRMADO) ';

	$where1 .= ' '.$str_fechas1;
	$where2 .= ' AND '.$str_fechas2;
	$where_acumulado1 .= ' '.$str_fechas_acumulado1;
	$where_acumulado2 .= '  AND  '.$str_fechas_acumulado2;

$sql = "SELECT ID,TIPO,NOMBRE,ID_COTIZACION,SUM(TOTALG) AS TOTALG,SUM(TOTALE) AS TOTALE,SUM(ACUMG) AS ACUMG,SUM(ACUME) AS ACUME, SUM(ACTIVAS) AS ACTIVAS  FROM(
SELECT P.ID, P.NOMBRE,C.ID AS ID_COTIZACION,
CASE 
...
CASE  
  ...
  END 
  END 
  END  AS TOTALE,0 AS ACUMG,0 AS ACUME,0 AS ACTIVAS
...
CASE  
  ...
  END 
  END 
  END  AS TOTALG,0 AS TOTALE,0 AS ACUMG,0 AS ACUME,0 AS ACTIVAS
   FROM PROSPECTO P INNER JOIN COTIZACIONES C ON P.ID = C.ID_PROSPECTO LEFT JOIN COTIZACIONES_STATUS_FECHA CSF ON C.ID = CSF.ID_COTIZACION WHERE C.ESTADO_COTIZACION = 4  ".$where2." 
	 UNION ALL
	 SELECT P.ID, P.NOMBRE,C.ID AS ID_COTIZACION,
CASE 
	WHEN C.BANDERA = 0 THEN
...
CASE  
  ...
  END 
  END 
  END  AS ACUME,0 AS ACTIVAS
  FROM PROSPECTO P INNER JOIN COTIZACIONES C ON P.ID = C.ID_PROSPECTO INNER JOIN COTIZACIONES_STATUS_FECHA CSF ON C.ID = CSF.ID_COTIZACION WHERE C.ESTADO_COTIZACION = 2 AND ".$where_acumulado1."
	UNION ALL
	SELECT P.ID, P.NOMBRE,C.ID AS ID_COTIZACION,
CASE 
	WHEN C.BANDERA = 0 THEN
...
CASE  
  ...
  END 
  END 
  END  AS ACUMG,0 AS ACUME,0 AS ACTIVAS
   FROM PROSPECTO P INNER JOIN COTIZACIONES C ON P.ID = C.ID_PROSPECTO LEFT JOIN COTIZACIONES_STATUS_FECHA CSF ON C.ID = CSF.ID_COTIZACION WHERE C.ESTADO_COTIZACION = 4  ".$where_acumulado2."
	 UNION ALL
SELECT P.ID, P.NOMBRE,C.ID AS ID_COTIZACION,
CASE 
	WHEN C.BANDERA = 0 THEN
...
END 
END AS TIPO,
0 AS TOTALG,0 AS TOTALE,0 AS ACUMG,0 AS ACUME,
CASE  
	WHEN C.ID_TIPO_SERVICIO = 1 || C.ID_TIPO_SERVICIO = 2 || C.ID_TIPO_SERVICIO = 12 || C.ID_TIPO_SERVICIO = 20 THEN
		(SELECT SUM(CT.MONTO) AS TOTAL FROM COTIZACIONES_TRAMITES CT WHERE C.ID = CT.ID_COTIZACION GROUP BY CT.ID_COTIZACION ORDER BY CT.ID_COTIZACION)  
	ELSE
  ...
  END 
  END  AS ACTIVAS
  FROM PROSPECTO P INNER JOIN COTIZACIONES C ON P.ID = C.ID_PROSPECTO INNER JOIN COTIZACIONES_STATUS_FECHA CSF ON C.ID = CSF.ID_COTIZACION WHERE (C.ESTADO_COTIZACION = 1 OR C.ESTADO_COTIZACION = 2 OR C.ESTADO_COTIZACION = 3 OR C.ESTADO_COTIZACION = 8 OR C.ESTADO_COTIZACION = 9) AND ".$where_acumulado1."
	 ) tt
GROUP BY ID,TIPO 
ORDER BY ID
";
