<?php
	...
	function count_sitios($ID_COTIZACION, $CONST){
		...
		$SITIO_MATRIZ =  $database->query("
			...
			FROM COTIZACION_SITIOS_INF_COM
	...
	/*	if($SITIO_MATRIZ[0]["COUNT_MATRIZ"] == 0){
			...
			$COUNT_SITIOS["SITIOS_A_VISITAR"] = $database->count("COTIZACION_SITIOS_INF_COM", ["ID_COTIZACION"=>$ID_COTIZACION]); 
			valida_error_medoo_and_die(); 
			//
			$COUNT_SITIOS["TOTAL_SITIOS"] = $database->count("COTIZACION_SITIOS_INF_COM", ["AND" => ["ID_COTIZACION"=>$ID_COTIZACION, "SELECCIONADO"=>1]]); 
			valida_error_medoo_and_die(); 
			if($COUNT_SITIOS["TOTAL_SITIOS"]>1){
				$COUNT_SITIOS["SITIOS_ID"] = $database->select("COTIZACION_SITIOS_INF_COM","*", ["AND" => ["ID_COTIZACION"=>$ID_COTIZACION, "SELECCIONADO"=>1]]); 
			}
