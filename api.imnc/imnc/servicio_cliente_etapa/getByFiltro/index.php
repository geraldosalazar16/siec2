...
for ($i=0; $i < count($servicio_cliente_etapa) ; $i++) {
	...
	{
		$curso = $database->query("SELECT C.ID_CURSO,C.NOMBRE as ID_NORMA ,SC.CANTIDAD_PARTICIPANTES FROM SCE_CURSOS SC INNER JOIN CURSOS C ON SC.ID_CURSO = C.ID_CURSO WHERE SC.ID_SCE =".$servicio_cliente_etapa[$i]["ID"].$QUERY3." LIMIT 1")->fetchAll(PDO::FETCH_ASSOC);
		$servicio_cliente_etapa[$i]["NORMAS"] = $curso;
        if($QUERY3=="")
		{
			if($QUERY1=="" && ($QUERY!="" || $QUERY==""))
			{
				$aux[] = $servicio_cliente_etapa[$i];
			}
		}
        else
		{
		...
		$normas = $database->query("SELECT DISTINCT ID_NORMA FROM SCE_NORMAS WHERE ID_SCE= ".$servicio_cliente_etapa[$i]["ID"].$QUERY1)->fetchAll(PDO::FETCH_ASSOC);
		$servicio_cliente_etapa[$i]["NORMAS"] = $normas;

		if($QUERY1=="")
		{
			if($QUERY3=="" && ($QUERY!="" || $QUERY==""))
			{
				$aux[] = $servicio_cliente_etapa[$i];
			}
		}
