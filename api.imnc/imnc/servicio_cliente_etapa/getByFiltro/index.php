...
if($ID_SECTOR != "" && $ID_SECTOR != "TODOS"){
	$whereID_SECTOR = " SG_SECTORES.ID_SECTOR = ".$ID_SECTOR;
	$where .= $whereID_SECTOR;
}
$whereNOMBRE_CLIENTE = "";
if ($NOMBRE_CLIENTE != "") {
	$likeNOMBRE_CLIENTE = "";
	...
	$whereNOMBRE_CLIENTE = ($where?" AND ":"")." CLIENTES.NOMBRE LIKE " . $database->quote($likeNOMBRE_CLIENTE . $NOMBRE_CLIENTE."%");
    $where .= $whereNOMBRE_CLIENTE; 
}
...
if ($NOMBRE_SERVICIO != "") {
	...
	$whereNOMBRE_SERVICIO = ($where?" AND ":"")." SERVICIOS.NOMBRE LIKE " . $database->quote($likeNOMBRE_SERVICIO . $NOMBRE_SERVICIO."%");
     $where .= $whereNOMBRE_SERVICIO;
...
if ($REFERENCIA != "") {
	...
	$whereREFERENCIA = ($where?" AND ":"")." SERVICIO_CLIENTE_ETAPA.REFERENCIA LIKE " . $database->quote($likeREFERENCIA . $REFERENCIA."%");
    $where .= $whereREFERENCIA;
}

//print_r($whereNOMBRE_CLIENTE.$whereNOMBRE_SERVICIO.$whereREFERENCIA.$whereID_SECTOR);
//exit();
