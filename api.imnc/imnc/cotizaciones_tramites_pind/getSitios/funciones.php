<?php
	...
	/*	if($SITIO_MATRIZ[0]["COUNT_MATRIZ"] == 0){
			...
			$COUNT_SITIOS["SITIOS_A_VISITAR"] = $database->count("COTIZACION_SITIOS_PIND", ["ID_COTIZACION"=>$ID_COTIZACION]); 
			valida_error_medoo_and_die(); 
			//
			$COUNT_SITIOS["TOTAL_SITIOS"] = $database->count("COTIZACION_SITIOS_PIND", ["AND" => ["ID_COTIZACION"=>$ID_COTIZACION, "SELECCIONADO"=>1]]); 
			valida_error_medoo_and_die(); 
			if($COUNT_SITIOS["TOTAL_SITIOS"]>1){
				$COUNT_SITIOS["SITIOS_ID"] = $database->select("COTIZACION_SITIOS_PIND","*", ["AND" => ["ID_COTIZACION"=>$ID_COTIZACION, "SELECCIONADO"=>1]]); 
			}
