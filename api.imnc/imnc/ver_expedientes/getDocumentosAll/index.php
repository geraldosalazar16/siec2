...
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
$servicio_cliente_etapa = $database->get("SERVICIO_CLIENTE_ETAPA", "*", ["ID"=>$id_servicio_cliente_etapa]); 
$id_tipo_serv=$servicio_cliente_etapa["ID_TIPO_SERVICIO"];
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
...
//	$DOCUMENTOS = $database->select("CATALOGO_DOCUMENTOS", "*",["SECCION"=>$nombre_seccion,"ORDER"=>"ID"]);
	$DOCUMENTOS = $database->select("CATALOGO_DOCUMENTOS",["[><]ETAPAS_PROCESO"=>["CATALOGO_DOCUMENTOS.ID_ETAPA"=>"ID_ETAPA"],"[><]CATALOGO_SECCIONES"=>["CATALOGO_DOCUMENTOS.ID_SECCION"=>"ID"]],["CATALOGO_DOCUMENTOS.ID","CATALOGO_DOCUMENTOS.NOMBRE","CATALOGO_DOCUMENTOS.DESCRIPCION","ETAPAS_PROCESO.ETAPA","CATALOGO_SECCIONES.NOMBRE_SECCION"],["AND"=>["CATALOGO_SECCIONES.NOMBRE_SECCION"=>$nombre_seccion,"CATALOGO_DOCUMENTOS.ID_TIPO_SERVICIO"=>$id_tipo_serv],"ORDER"=>"CATALOGO_DOCUMENTOS.ID"]);
}
else{
	//$DOCUMENTOS = $database->select("CATALOGO_DOCUMENTOS", "*",["AND"=>["ETAPA"=>$nombre_etapa,"SECCION"=>$nombre_seccion],"ORDER"=>"ID"]);
	$DOCUMENTOS = $database->select("CATALOGO_DOCUMENTOS",["[><]ETAPAS_PROCESO"=>["CATALOGO_DOCUMENTOS.ID_ETAPA"=>"ID_ETAPA"],"[><]CATALOGO_SECCIONES"=>["CATALOGO_DOCUMENTOS.ID_SECCION"=>"ID"]],["CATALOGO_DOCUMENTOS.ID","CATALOGO_DOCUMENTOS.NOMBRE","CATALOGO_DOCUMENTOS.DESCRIPCION","ETAPAS_PROCESO.ETAPA","CATALOGO_SECCIONES.NOMBRE_SECCION"] ,["AND"=>["ETAPAS_PROCESO.ETAPA"=>$nombre_etapa,"CATALOGO_SECCIONES.NOMBRE_SECCION"=>$nombre_seccion,"CATALOGO_DOCUMENTOS.ID_TIPO_SERVICIO"=>$id_tipo_serv],"ORDER"=>"CATALOGO_DOCUMENTOS.ID"]);
}

for($i=0;$i<count($DOCUMENTOS);$i++){
	$ESTADO_DOCUMENTO = $database->select("BASE_DOCUMENTOS",["ESTADO_DOCUMENTO","EXTENSION_DOCUMENTO","UBICACION_DOCUMENTOS"],["AND"=>["ID_CATALOGO_DOCUMENTOS"=>$DOCUMENTOS[$i]["ID"],"ID_SERVICIO"=>$id_servicio_cliente_etapa,"CICLO"=>$nombre_ciclo]]);
	$DOCUMENTOS[$i]["ESTADO"]=$ESTADO_DOCUMENTO[0]['ESTADO_DOCUMENTO'];
	$DOCUMENTOS[$i]["EXTENSION"]=$ESTADO_DOCUMENTO[0]['EXTENSION_DOCUMENTO'];
	$DOCUMENTOS[$i]["UBICACION"]=$ESTADO_DOCUMENTO[0]['UBICACION_DOCUMENTOS'];
