<?php
	...
	SERVICIOS.ACRONIMO AS ACRONIMO_SERVICIO,
    TIPOS_SERVICIO.ID AS ID_TIPO_SERVICIO,
	TIPOS_SERVICIO.ACRONIMO AS ACRONIMO_TIPO_SERVICIO,
	...
	LEFT JOIN PROSPECTO_ESTATUS_SEGUIMIENTO ON PROSPECTO_PRODUCTO.ID_ESTATUS_SEGUIMIENTO=PROSPECTO_ESTATUS_SEGUIMIENTO.ID
    WHERE
    ...
    for($i=0;$i<count($prospecto_productos);$i++){
        ...
        if($prospecto_productos[$i]["ID_SERVICIO"]==3)
        {
            $query = 
            "SELECT 
            (CASE WHEN PPC.MODALIDAD = 'programado' THEN PPC.ID_CURSO_PROGRAMADO 
            ELSE 
            CASE WHEN PPC.MODALIDAD = 'insitu' THEN PPC.ID_CURSO 
            END 
            END) 
            AS ID_CURSO,
            PPC.MODALIDAD,
            (CASE WHEN PPC.MODALIDAD = 'programado' THEN 
            (SELECT CONCAT('Curso Programado: ',C.NOMBRE)  
            FROM CURSOS C INNER JOIN CURSOS_PROGRAMADOS CP 
            ON C.ID_CURSO = CP.ID_CURSO 
            WHERE PPC.ID_CURSO_PROGRAMADO = CP.ID) 
            ELSE 
            CASE WHEN PPC.MODALIDAD = 'insitu' THEN
            (SELECT CONCAT('Curso: ',C.NOMBRE)  
            FROM CURSOS C  WHERE PPC.ID_CURSO = C.ID_CURSO)
            END 
            END) 
            AS NOMBRE_CURSO,
            CANTIDAD_PARTICIPANTES AS CANTIDAD, 
            SOLO_PARA_CLIENTE
            FROM PROSPECTO_PRODUCTO_CURSO PPC 
            WHERE PPC.ID_PRODUCTO =".$prospecto_productos[$i]['ID'];
