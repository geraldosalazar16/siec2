...
$strQuery = <<<EOT
SELECT CLIENTES.ID, CLIENTES.ID_TIPO_PERSONA, CLIENTES.ID_TIPO_ENTIDAD, 
	CLIENTES.ID_CLIENTE_FACTURARIO, CLIENTES.NOMBRE, CLIENTES.RFC, CLIENTES.ES_FACTURARIO, CLIENTES.TIENE_FACTURARIO, 
	CLIENTES.IMAGEN_BASE64, CLIENTES.CLIENTE_FACTURARIO, CLIENTES.RFC_FACTURARIO,COUNT(PROSPECTO.ID) AS TOTAL,GROUP_CONCAT(PROSPECTO.ID) AS PROSPECTOS 
FROM CLIENTES $fromCLIENTES_DOMICILIOS $fromCLIENTES_CONTACTOS LEFT JOIN PROSPECTO  ON CLIENTES.ID = PROSPECTO.ID_CLIENTE
WHERE 1 
	$whereCLIENTES_DOMICILIOS  
	$whereCLIENTES_CONTACTOS 
	$whereNOMBRE 
