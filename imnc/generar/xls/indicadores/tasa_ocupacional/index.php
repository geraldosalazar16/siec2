...
/*
	* A PARTIR DE AQUI CALCULO LA TASA OCUPACIONAL
	* BUSCO EL REPORTE DE TASA OCUPACIONAL UTILIZANDO LA API PARA ESTO
*/	
	$ruta = $global_apiserver.'/i_reportes/getTasaOcupacionalAuditores/';
	$tablaDatos = file_get_contents($ruta);
	$tablaDatos = json_decode($tablaDatos);
...
$hasta = 1;
$fila = 3;
$objWorkSheet->setCellValue('A1', "REPORTE DE TASA OCUPACIONAL ".$texto_mes." ".$anhio);
...
foreach ($tablaDatos as $key=>$row)
{
    ...
    $objWorkSheet->setCellValue($ABCD[1].$fila, $row->TASA_OCUPACIONAL[$mes]);
...
header('Content-Disposition: attachment;filename="reporte_tasa_ocupacional_'.$texto_mes.'_'.$anhio.'.xlsx"');
