...
app.controller('tabla_entidades_controller', ['$scope', function($scope) { 
	//Titulo que aparece en el html
	$scope.titulo = 'Tabla de Entidades';
	/*
	...
	*/
	$scope.actualizaTabla = function(){
		$.ajax({
			type:'GET',
			url:global_apiserver + "/ex_tabla_entidades/getAll/",
			success: function(data){
				$scope.$apply(function(){
					$scope.tablaentidades = angular.fromJson(data);
				});
			}
		});
	};
	...
	*/
	$scope.editar =  function(entidad_id) {		
		$("#btnGuardar").attr("accion","editar");
		$("#modalTitulo").html("Editar Tipo de Documento");
	  
		$.getJSON( global_apiserver + "/ex_tabla_entidades/getById/?id="+entidad_id, function( response ) {
			$scope.id = response.ID;
			$scope.nombre = response.TABLA;
		...
		if($scope.nombre.length > 0){	
			$.ajax({
				...
				success: function(data){
					if(data.cantidad > 0){
						$scope.respuesta =  0;	
						$("#nombreerror").text("Nombre de la entidad ya registrada");						
					}else{
						$("#nombreerror").text("");
					}
				}
	...
	$scope.guardar = function() {		
		if ($("#btnGuardar").attr("accion") == "insertar")
		{
			...
			if($scope.respuesta == 1){
				...
				$.post(global_apiserver + "/ex_tabla_entidades/insert/", JSON.stringify(entidad), function(respuesta){
		...
		else if ($("#btnGuardar").attr("accion") == "editar")
		{
			...
			if($scope.respuesta == 1){
				...
				$.post( global_apiserver + "/ex_tabla_entidades/update/", JSON.stringify(entidad), function(respuesta){
					respuesta = JSON.parse(respuesta);
					if (respuesta.resultado == "ok") {
						$("#modalInsertarActualizar").modal("hide");
