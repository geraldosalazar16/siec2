...
function insertar_contacto(id_domicilio) {
  ...
  $.post(global_apiserver + "/clientes_contactos/insert/", JSON.stringify(cliente_domicilio_contacto), function (respuesta) {
    ...
    if (respuesta.resultado == "ok") {
      ...
      draw_domicilios();
      insertar_cliente_contactos_servicios(respuesta.id);
...
function editar_contacto(id, id_domicilio) {
  ...
  $.post(global_apiserver + "/clientes_contactos/update/", JSON.stringify(cliente_domicilio_contacto), function (respuesta) {
    ...
    if (respuesta.resultado == "ok") {
      ...
      draw_domicilios();
      //funcion para actualizar en tabla cliente_contactos_servicios pasandole el ID
      actualizar_cliente_contactos_servicios(parseInt(id));
    }
    else {
      notify("Error", respuesta.mensaje, "error");
    }
...
//llena el multiselect Con los servicios disponibles para  que a la hora de crear un contacto pueda serle asignado el/los Servicios
function listar_Servicios() {
  $.getJSON(global_apiserver + "/servicios/getAll/", function (response) {
    $.each(response, function (indice, objServicio) {
      $("#idservicio").append('<option value="' + objServicio.ID + '">' + objServicio.NOMBRE + '</option>');
    });
    $('#idservicio').select2();
  });
}
function insertar_cliente_contactos_servicios(id_contacto) {
  var cliente_contactos_servicios = {
...
function actualizar_cliente_contactos_servicios(id_contacto) {
  var cliente_contactos_servicios = {
    ID_CONTACTO: parseInt(id_contacto),
    ID_SERVICIO: $("#idservicio").val()
  };
